require 'nokogiri'
require 'open-uri'
class WatchonlinemovieController < ApplicationController
	...
	def get_latest_movies
		...
		site_url = 'http://www.watchonlinemovies.com.pk'
	...
	def page_urls
		...
		for i in 1..total_pages
			url_array << site_url + '/page/'+i.to_s+'/'
		end 
		return url_array
	end 
	...
	def get_movie_divs_from_raw_data(doc) #that function will only returns the total movie divs on a single page 
		...
		doc = doc.css('#content').css('.postbox')
	end 


	def get_movie_information_from_a_div(movie_div) #that function will return hash of a single movies data
	    movie_data = Hash.new
        movie_data[:movie_name]      = movie_div.css('a')[0]['title']
        movie_data[:movie_url]       = movie_div.css('a')[0]['href'] 
        movie_data[:movie_image_url] = !movie_div.css('img').empty? ? movie_div.css('img')[0]['src'] : 'https://www.oatey.com/ASSETS/WEB_THEMES//OATEY/images/NoImage.png'
