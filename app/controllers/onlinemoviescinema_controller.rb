require 'nokogiri'
require 'open-uri'
class OnlinemoviescinemaController < ApplicationController
	def index 
	...
	def get_latest_movies
		...
		site_url = 'http://onlinemoviescinema.cc'
	...
	def page_urls
		...
		for i in 1..total_pages
			url_array << site_url + '/movies/page/'+i.to_s+'/'
		end 
		return url_array
	end 
	...
	def get_movie_divs_from_raw_data(doc) #that function will only returns the total movie divs on a single page 
		...
		doc = doc.css('.main-content').css('.video-section').css('.responsive-height')
	end 
